


    <div class="card mb-3 mb-md-4">

        <div class="card-body">
            <!-- Breadcrumb -->
            <nav class="d-none d-md-block" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="#">Equipos</a>
                    </li>
                    <li class="breadcrumb-item active" aria-current="page">Todos los equipos</li>
                </ol>
            </nav>
            <!-- End Breadcrumb -->
      
            <button id="btn_actualizar_varios_estadosequipos" type="submit" class="btn btn-primary float-right">Cambiar estado</button>
            <!--  -->
            <div class="mb-3 mb-md-4 d-flex justify-content-between">
                <div class="h3 mb-0">Equipos</div>
            </div>


            <!-- Users -->
            <div class="table-responsive-xl" >
                <table class="table text-nowrap mb-0">
                    <thead >
                        <tr>
                            <th class="font-weight-semi-bold border-top-0 py-3">#</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">tipo</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">marca</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">serial</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">direccion_mac_wifi</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">direccion_mac_ethenet</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">imei1</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">imei2</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">F. creacion</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">estado</th>
                            <th class="font-weight-semi-bold border-top-0 py-3">acciones</th>
                        </tr>
                    </thead>
                    <tbody id="tabla_verequipos">
                        <tr>

                            <td class="text-center"> NO HAY DATOS </td>
                        </tr>
                        
                    </tbody>
                </table>



                <div class="card-footer d-block d-md-flex align-items-center d-print-none">
                    <div class="d-flex mb-2 mb-md-0">Showing 1 to 8 of 24 Entries</div>

                    <nav class="d-flex ml-md-auto d-print-none" aria-label="Pagination"><ul class="pagination justify-content-end font-weight-semi-bold mb-0">				<li class="page-item">				<a id="datatablePaginationPrev" class="page-link" href="#!" aria-label="Previous"><i class="gd-angle-left icon-text icon-text-xs d-inline-block"></i></a>				</li><li class="page-item d-none d-md-block"><a id="datatablePaginationPage0" class="page-link active" href="#!" data-dt-page-to="0">1</a></li><li class="page-item d-none d-md-block"><a id="datatablePagination1" class="page-link" href="#!" data-dt-page-to="1">2</a></li><li class="page-item d-none d-md-block"><a id="datatablePagination2" class="page-link" href="#!" data-dt-page-to="2">3</a></li><li class="page-item">				<a id="datatablePaginationNext" class="page-link" href="#!" aria-label="Next"><i class="gd-angle-right icon-text icon-text-xs d-inline-block"></i></a>				</li>				</ul></nav>
                </div>
            </div>
            <!-- End Users -->
        </div>
    </div>



